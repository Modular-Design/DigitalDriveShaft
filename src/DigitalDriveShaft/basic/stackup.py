...
import numpy as np
from .ply import Ply
from .material import TransverselyIsotropicMaterial
...
class Stackup:
    ...
    def get_strains(self, deformation: np.ndarray) -> List[Tuple[np.ndarray, np.ndarray]]:
        """
    ...
    def get_stresses(self, strains: List[Tuple[np.ndarray, np.ndarray]]) -> List[Tuple[np.ndarray, np.ndarray]]:
        """
