...
def Torsionsfestigkeit(D_Trennfuge, L_Welle, Last, Mat_Welle, Layup, Sicherheit: Optional[float] = 1.0):
    ...
    D_Mittelflaeche = D_Trennfuge + sum(thickness)
...
# =============================================================================
    ...
    print(f'Wandstärke der Diagonalschicht für Torsionsfestigkeit: {round(t_Diagonallagen, 1)} mm')
    print(f'Sicherheit gegen Torsionsbruch: {round(Sicherheit_Torsionsbruch, 1)}')
...
def Beulfestigkeit(D_Trennfuge, L_Welle, Last, Mat_Welle, Layup, Sicherheit: Optional[float] = 1.0):
    ...
    Beulmoment = k_s * k_l * np.pi ** 3 / 6 * (D_Welle_Außen / 2) ** (5 / 4) * t_Diagonallagen ** (9 / 4) / np.sqrt(
    ...
    if Sicherheit_TB < Sicherheit:
        ...
        if Auslegungsziel == 'Steifigkeit':
            ...
            while abs(Last.Mx * Sicherheit - Beulmoment) > 0.1:
                ...
                thickness = [t_Diagonallagen / 4, t_Diagonallagen / 4, t_Umfangslagen / 2, t_Axiallagen,
                             t_Umfangslagen / 2, t_Diagonallagen / 4, t_Diagonallagen / 4]
                ...
                Beulmoment = k_s * k_l * np.pi ** 3 / 6 * (D_Welle_Außen / 2) ** (5 / 4) * t_Gesamt ** (
                            9 / 4) / np.sqrt(L_Welle) * E_Axial ** (3 / 8) * (E_Umfang / (1 - Mat_Welle.Ny12 ** 2)) ** (
                                         5 / 8) / 1000
        ...
        if Auslegungsziel == 'Fertigung':
            ...
            t_Diagonallagen_Beulen = ((6 * Last.Mx * 1000 * Sicherheit * L_Welle ** (1 / 2)) / (
                        k_s * k_l * np.pi ** 2 * E_Axial ** (3 / 8) * (D_Welle_Außen / 2) ** (5 / 4)) * (
                                                  E_Umfang / (1 - Mat_Welle.Ny12 ** 2)) ** (-5 / 8)) ** (4 / 9)
            ...
            while abs(Last.Mx * Sicherheit - Beulmoment) > 0.1:
                ...
                thickness = [t_Diagonallagen_Beulen / 4, t_Diagonallagen_Beulen / 4, t_Umfangslagen / 2, t_Axiallagen,
                             t_Umfangslagen / 2, t_Diagonallagen_Beulen / 4, t_Diagonallagen_Beulen / 4]
                t_Gesamt = sum(thickness)
                D_Welle_Außen = D_Trennfuge + 2 * t_Gesamt
                ...
                # Beulmomentberechnung nach Excel von Sebastian
                Beulmoment = k_s * k_l * np.pi ** 3 / 6 * (D_Welle_Außen / 2) ** (5 / 4) * t_Gesamt ** (
                            9 / 4) / L_Welle ** (1 / 2) * E_Axial ** (3 / 8) * (
                                         E_Umfang / (1 - Mat_Welle.Ny12 ** 2)) ** (5 / 8) / 1000
            ...
            print(f"Erhöhung der Diagonallagen gegen Torsionsbeulen um {round(t_Erhoehung_Beulen, 1)} mm")
            Beulmoment = k_s * k_l * np.pi ** 3 / 6 * (D_Welle_Außen / 2) ** (5 / 4) * t_Gesamt ** (9 / 4) / np.sqrt(
                L_Welle) * E_Axial ** (3 / 8) * (E_Umfang / (1 - Mat_Welle.Ny12 ** 2)) ** (5 / 8) / 1000
            Sicherheit_TB = Beulmoment / Last.Mx
            print(f"Sicherheit gegen Torsionsbeulen: {round(Sicherheit_TB, 1)}")
