...
mapdl.run("pi = acos(-1)                 ")  # Definition von pi

mapdl.run("drehzahl = 0                  ")  # Drehzahl in [1/min]

mapdl.run("winkelg = (drehzahl/60)*2*pi  ")  # Winkelgeschwindigkeit in [rad/s]

mapdl.run("nz = 4                        ")  # Zaehneanzahl

mapdl.run("sw = 360/nz/2                 ")  # Halber Sektionswinkel in [deg]

mapdl.run("ra = 97.5                       ")  # Aussenradius in [mm]

mapdl.run("la = 12                    ")  # Dicke der Aussenlage in [mm]

mapdl.run("li = 2                     ")  # Dicke der Innenlage in [mm]

mapdl.run("zw = 0.12*sw                  ")  # Anteil des Zahnkopfs am Sektor in [deg]

mapdl.run("zh = 6		                ")  # Zahnhoehe in [mm]

mapdl.run("zf = 0.63*sw                  ")  # Anteil Zahnfuss in [deg]

mapdl.run("rk = 0.5                      ")  # Radius am Zahnkopf in [mm]

mapdl.run("rf = 0.5                      ")  # Radius am Zahnfuss in [mm]

mapdl.run("bw = 0.8*sw                 ")  # Zahnfussbreite in [deg]

mapdl.run("deep = 40                     ")  # Laenge des Wellenstuecks in [mm]

mapdl.run("deepin = 20")

mapdl.run("deepout = 20")

mapdl.run("mom = 168000000               ")  # Drehmoment in [Nmm]

mapdl.run("alpha = 45                    ")  # Zahnflankenwinkel in [deg]

mapdl.run("dnabe = 7")

mapdl.run("rinabe = ra-la-li-zh-dnabe						")  # Radius der Nabe in [mm]
mapdl.run("sb = ra-la-li")
mapdl.run("sa = ra-la-li-zh/2")
mapdl.run("wb = (180-alpha)*pi/180")
mapdl.run("x =  sa/sb*sin(wb)")
mapdl.run("wa = asin(x)")
mapdl.run("wa = wa*180/pi")
mapdl.run("wb = wb*180/pi")
mapdl.run("fw = 180-wa-wb")
mapdl.run("fw = bw-fw")
with mapdl.non_interactive:
    mapdl.run("*if,li,gt,la,then")
    mapdl.run("es = la")
    mapdl.run("*else")
    mapdl.run("es = li")
    mapdl.run("*endif")
mapdl.run("es=0.265*3")
# es=0.265/2
# es=0.265/3
mapdl.prep7()
...
CFK_UD_HTS40.add_to_mapdl(mapdl, 201)
cfk_failure.add_to_mapdl(mapdl, 201)

CFK_UD_HTS40.add_to_mapdl(mapdl, 202)
cfk_failure.add_to_mapdl(mapdl, 202)
# mapdl.run("CFK_UD_HTS40,201")  # TODO: enable later
# mapdl.run("CFK_BD_HTS40,202")  # TODO: enable later
...
mapdl.nummrg("node")
mapdl.allsel()
mapdl.k(111, 0, 0, "-deepout")
mapdl.lsel("s", "line", "", 18)
mapdl.lsel("a", "line", "", 7)
mapdl.lsel("a", "line", "", 4)
mapdl.lgen(2, "all", "", "", "", "", "-deepout", "", 1, 0)
mapdl.run("alls")
# rins=ra-2*(la+li+zh)
# k,112,0,rins,-deep/4
# k,112,0,ra-la-li-zh*3,-deepout
mapdl.k(112, 0, "rinabe", "-deepout")
...
mapdl.vext(41, "", "", 0, 0, "deepin+deepout")
mapdl.allsel()
mapdl.vsel("s", "volu", "", 7)
...
mapdl.mp("ex", 204, 210e3)
mapdl.mp("prxy", 204, 0.3)
mapdl.mp("dens", 204, "7.8-9") # TODO: 7.8e-9?
mapdl.aesize(45, "es")
mapdl.aesize(44, "es")
mapdl.aesize(46, "es")
...
# mapdl.lplot()  # TODO: delete later
# mapdl.vplot()  # TODO: delete later
# ==============================================================================
# Kontakte
mapdl.mp("MU", 201, "")
mapdl.mat(201)
...
mapdl.d("all", "uz", 0)
mapdl.f("all", "mz", "mom/nz")
mapdl.allsel()
mapdl.cyclic()
# mapdl.eplot()
mapdl.lplot()  # TODO: delete later
mapdl.eplot()  # TODO: delete later
mapdl.run("/solu")
