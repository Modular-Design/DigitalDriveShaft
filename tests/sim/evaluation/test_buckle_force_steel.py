...
steel = IsotropicMaterial(200000,   # [t s^-2 mm^-1]
                          0.3,   # []
                          7.85e-09)  # [t mm^-3]
...
def test_buckle_force(l_thickness, ds_diameter, ds_length, fkrit):
    ...
    shaft = SimpleDriveShaft(diameter=ds_diameter, length=ds_length, stackup=stackup, contour=EContour.CENTER)
    result = calc_buckling(mapdl, shaft, dict(n_phi=20, n_z=50), "FORCE") # dict(n_phi=20, n_z=50)
