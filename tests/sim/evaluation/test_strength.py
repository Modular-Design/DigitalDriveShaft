...
@pytest.mark.parametrize(
    ...
    [
        (1.0, [0], 10, 30, analysis.Loading(fz=100), 285.93),  # fz in N
        (1.0, [90], 10, 30, analysis.Loading(fz=100), 19.44),  # fz in N
        (1.0, [0], 10, 30, analysis.Loading(fz=1000), 28.593),
    ],
)
def test_axial_laoding(
    l_thickness, l_orientations, ds_diameter, ds_length, loading, safety
):
...
@pytest.mark.parametrize(
    ...
    [
        (1.0, [90], 10, 30, analysis.Loading(fz=100)),
        (1.0, [0, 90], 10, 30, analysis.Loading(fz=100)),
        (1.0, [0], 10, 30, analysis.Loading(mz=100)),
        (10.1 / 4, [45, -45, -45, 45], 85.5 * 2, 100, analysis.Loading(mz=168960)),
        (
            11.02 / 7,
            [45, -45, 90, 0, 90, -45, 45],
            79.42 * 2,
            400,
            analysis.Loading(mz=20000),
        ),
    ],
)
